...
export async function GET(req: NextRequest) {
  ...
  if (!ticker || !/^[A-Z]{1,5}$/.test(ticker)) {
    const response: APIResponse<null> = { ok: false, data: null, error: 'Invalid ticker. Pass ?ticker=AAPL' };
    return NextResponse.json(response, { status: 400 });
  }

  try {
    const [priceResult, ohlcResult, spyPrice, qqqPrice, spyOHLC, qqqOHLC] = await Promise.all([
    ...
    regimeMeta.stale = spyPrice.meta.stale || qqqPrice.meta.stale || spyOHLC.meta.stale || qqqOHLC.meta.stale;

    const regime = computeRegime(spyOHLC.data, qqqOHLC.data, spyPrice.data, qqqPrice.data, regimeMeta);

    const bars = ohlcResult.data;
    const price = priceResult.data.price;

    const atrVal = bars.slice(-14).reduce((acc, bar, i, arr) => {
      if (i === 0) return acc;
      const prev = arr[i - 1];
    ...
    const trade = scoreSetup(ticker, bars, regime, direction, entry, stop, target, ohlcResult.meta);
